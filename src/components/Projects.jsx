...
export default function Projects() {
  const projects = [
    ...
    {
      name: "SecurePay",
      logo: "https://www.dropbox.com/scl/fi/920y4m32v7kymjkg5oy2e/SP.png?rlkey=x3wvmc6qjo6hac5uy8ckkbl2w&st=b8ilcsp9&raw=1",
      domain: "Full Stack",
      date: "May 2025",
    ...
    {
      name: "SmartMusic",
      logo: "https://www.dropbox.com/scl/fi/ag75ojz4aep6jtsqux3lf/SM.png?rlkey=830u06j78rs6scd4tstjxe0rn&st=j6dt5dt4&raw=1",
      domain: "Python AI-ML",
      date: "January 2025",
    ...
    {
      name: "Library System",
      logo: "https://www.dropbox.com/scl/fi/52aqhx5s4nwbaozhmw743/LS.png?rlkey=hu8aoepf58tldfv5dwji066pn&st=hr8nxh41&raw=1",
      domain: "Java",
      date: "October 2024",
        ...
        {projects.map((proj, idx) => {
          ...
          return (
            <motion.div
              ...
              className="relative"
            >
              {/* Enhanced 3D-style Card Container */}
              <div
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-orange-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-700 rounded-2xl"></div>
                ...
                <div className="p-6 flex flex-col items-center text-center z-10 relative">
                  <div className="relative mb-4 transition-transform duration-500 group-hover:scale-110">
                    <div className="absolute -inset-1 bg-gradient-to-r from-orange-400 to-orange-600 rounded-full blur opacity-0 group-hover:opacity-30 transition-opacity duration-700"></div>
                    <img
                      src={proj.logo || "/placeholder.svg"}
                      alt={proj.name}
                      className="h-16 w-16 object-contain relative z-10"
                    />
                  </div>

                  <h3 className="text-lg sm:text-xl font-bold text-white mb-1 transition-transform duration-500 group-hover:scale-105">
                    {proj.name}
                  </h3>
                  ...
                  <div className="flex space-x-4 w-full justify-center">
                    ...
                    <AnimatedButton onClick={() => setOpenIndex(idx)} text="Details" color="white" />
                  </div>
                </div>
              </div>
              ...
              <AnimatePresence>
                {openIndex === idx && (
                  ...
                  >
                    <div className="absolute inset-0 bg-black/80" onClick={() => setOpenIndex(null)}></div>

                    <motion.div
                      initial={{ rotateX: 20 }}
                      animate={{ rotateX: 0 }}
                      className="relative bg-black-700 backdrop-blur-lg border border-white/20 rounded-2xl p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto z-10"
                    >
                      <button
                        onClick={() => setOpenIndex(null)}
                        className="absolute top-4 right-4 text-white text-2xl hover:text-orange-400 transition-colors"
                      >
                        ×
                      </button>
...
// Animated Button Component with left-to-right color transition
function AnimatedButton({ onClick, href, text, color = "orange", icon, external = false, size = "normal" }) {
  ...
  const baseClasses =
    "relative overflow-hidden rounded-md font-medium transition-all duration-300 flex items-center justify-center";
  ...
  const colorClasses = {
    ...
    white: "border border-orange-400 text-white hover:text-orange-400",
  };
  ...
  const ButtonContent = () => (
    <>
      <div
        className={`button-overlay absolute inset-0 ${overlayColors[color]} transform -translate-x-full transition-transform duration-500`}
      ></div>
  ...
  if (href) {
    return (
      <a
        ...
        className={`${baseClasses} ${sizeClasses} ${colorClasses[color]}`}
